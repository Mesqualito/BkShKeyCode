...
export class ItemService {
  ...
  protected convertDateFromClient(item: IItem): IItem {
    const copy: IItem = Object.assign({}, item, {
      timestamp: item.timestamp != null && item.timestamp.isValid() ? item.timestamp.toJSON() : null,
      modificationDate: item.modificationDate != null && item.modificationDate.isValid() ? item.modificationDate.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((item: IItem) => {
        item.timestamp = item.timestamp != null ? moment(item.timestamp) : null;
        item.modificationDate = item.modificationDate != null ? moment(item.modificationDate) : null;
      });
    }
    return res;
  }
